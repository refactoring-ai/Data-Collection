...
public class JGitUtils {
	...
	public static boolean isLocal(String gitUrl) {
	...
	// Local repositories without remote are formatted as: @local/repository/commit
	// Id
	// TODO: evaluate if this pattern works for other repo hosters as well, e.g.
	// BitBucket
	...
	/**
	 * In some cases, RMiner actually returns the 'new' path, even in its
	 * 'classesBeforeRefactoringMethod'. See issue
	 * https://github.com/refactoring-ai/predicting-refactoring-ml/issues/144 To
	 * counteract this, we then get a map with all old and new files, so that we can
	 * always use the old file to retrieve the version we actually extract features
	 * from.
	 *
	 * Note that key is the new file, and value is the old file. So, if we don't
	 * find the file in this map, this means it's already the old file.
	 */
	public static Map<String, String> getMapWithOldAndNewFiles(List<DiffEntry> entries) {
	...
	public static List<DiffEntry> calculateDiffEntries(RevCommit commit, DiffFormatter diffFormatter)
			throws IOException {
		RevCommit commitParent = commit.getParentCount() == 0 ? null : commit.getParent(0);
		return diffFormatter.scan(commitParent, commit);
	}
	...
	public static Set<ImmutablePair<String, String>> getJGitRenames(List<DiffEntry> entries) {
		...
		if (entries != null) {
			...
			renamedClassesJGit = renameEntries.stream().map(JGitUtils::getClassNames).collect(Collectors.toSet());
		}
	...
	// returns the filename before and after the rename
	public static Set<ImmutablePair<String, String>> getRefactoringMinerRenames(
		...
		if (refactoringsToProcess != null) {
			Set<Refactoring> renameRefactorings = refactoringsToProcess.stream().filter(RefactoringUtils::isClassRename)
					.collect(Collectors.toSet());
			renamedClassesRMiner = renameRefactorings.stream().map(JGitUtils::getClassNames).flatMap(Collection::stream)
					.collect(Collectors.toSet());
		}
	...
	private static Set<ImmutablePair<String, String>> getClassNames(Refactoring refactoring) {
		Set<ImmutablePair<String, String>> before = refactoredFilesAndClasses(refactoring,
				refactoring.getInvolvedClassesBeforeRefactoring());
		Set<ImmutablePair<String, String>> after = refactoredFilesAndClasses(refactoring,
				refactoring.getInvolvedClassesAfterRefactoring());
		if (before.size() != after.size())
			throw new IllegalStateException(
					"The size of the list of filenames before the refactoring is different from the size afterwards.");
